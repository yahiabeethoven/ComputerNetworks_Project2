...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (current_packet != recvpkt->hdr.seqno)                                       // if the packet received is not the exepcted packet in order, then continue because maybe it will come later
        {
            ...
            {
                sndpkt = make_packet(0);
                // regardless of the packet just received, send back an ACK of the currently expected packet
            ...
            else if (recvpkt->hdr.seqno > current_packet)                                    // the packet received is out-of-order, so buffer it
            {
